...
export default function Homepage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if(forceFetchAfterPost || forceFetchAfterDelete) {
    ...
    fetchData();
  }, [forceFetchAfterPost, forceFetchAfterDelete]);
  ...
  return (
    <div className={styles.container}>
      <div className={styles.subContainer}>
        ...
        <EventList setForceFetchAfterDelete={setForceFetchAfterDelete} eventsData={eventsData} setToastInfo={setToastInfo}/>
      </div>
    </div>
  )
}
