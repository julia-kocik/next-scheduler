...
export default function EventListItem({id, name, surname, email, date, setToastInfo, setForceFetchAfterUpdate, setForceFetchAfterDelete }: EventListItemProps) {
  ...
  const onUpdateHandler = async (): Promise<void> => {
    if(!showUpdateOptions) {
      return
        ...
        try {
          setIsUpdating(true)
          let url = `http://scheduler-env.eba-di2rddya.eu-north-1.elasticbeanstalk.com/api/v1/event/${id}?`
          ...
          if (error instanceof AxiosError && error.message) {
            if (error?.response?.data.message[0] === 'Invalid email format') {
              showToast('Invalid email format, please try again', 'red', setToastInfo);
            } else {
              showToast('Error occurred when data updating, please try again later.', 'red', setToastInfo);
            }
          } else {
